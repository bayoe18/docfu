...
describe('Build Cleanup', () => {
  ...
  it('should clean workspace on subsequent prepare', async () => {
    ...
    let result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    assert.strictEqual(result.exitCode, 0, 'First prepare should succeed')

    writeFileSync(join(paths.workspace, 'stale-file.md'), '# Stale\n\nThis should be removed')

    result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    assert.strictEqual(result.exitCode, 0, 'Second prepare should succeed')

    assert.ok(!existsSync(join(paths.workspace, 'stale-file.md')), 'Should remove stale files from workspace')
    assert.ok(existsSync(join(paths.workspace, 'index.md')), 'Should still have current files')
  })
  ...
  it('should handle removed source files', async () => {
    ...
    let result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    assert.strictEqual(result.exitCode, 0, 'First prepare should succeed')
    assert.ok(existsSync(join(paths.workspace, 'removed.md')), 'Should create removed.md')
    ...
    assert.ok(!existsSync(join(paths.workspace, 'removed.md')), 'Should remove file that was deleted from source')
    assert.ok(existsSync(join(paths.workspace, 'index.md')), 'Should keep remaining files')
  })
  ...
  it('should be idempotent (same input produces same output)', async () => {
    ...
    const firstIndex = readFileSync(join(paths.workspace, 'index.md'), 'utf-8')
    const firstGuide = readFileSync(join(paths.workspace, 'guide.mdx'), 'utf-8')
    const firstManifest = readFileSync(join(paths.workspace, 'manifest.json'), 'utf-8')

    result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    assert.strictEqual(result.exitCode, 0, 'Second prepare should succeed')

    const secondIndex = readFileSync(join(paths.workspace, 'index.md'), 'utf-8')
    const secondGuide = readFileSync(join(paths.workspace, 'guide.mdx'), 'utf-8')
    const secondManifest = readFileSync(join(paths.workspace, 'manifest.json'), 'utf-8')
  ...
  it('should handle format changes (md -> mdx)', async () => {
    ...
    let result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    assert.strictEqual(result.exitCode, 0, 'First prepare should succeed')
    assert.ok(existsSync(join(paths.workspace, 'page.md')), 'Should create .md file')
    ...
    assert.ok(existsSync(join(paths.workspace, 'page.mdx')), 'Should create .mdx file')
    assert.ok(!existsSync(join(paths.workspace, 'page.md')), 'Should remove old .md file')
  })
  ...
  it('should clean dist directory on full build', async () => {
    ...
    let result = await runCLI(['build', paths.source, '--workspace', paths.workspace, '--dist', paths.dist])
    ...
    result = await runCLI(['build', paths.source, '--workspace', paths.workspace, '--dist', paths.dist])
  ...
  it('should handle workspace directory already existing', async () => {
    ...
    writeFileSync(join(paths.workspace, 'another.md'), '# Old')

    const result = await runCLI(['prepare', paths.source, '--workspace', paths.workspace])
    ...
    assert.ok(!existsSync(join(paths.workspace, 'another.md')), 'Should remove all old files')
    assert.ok(existsSync(join(paths.workspace, 'index.md')), 'Should create new files')
  })
  ...
  it('should preserve workspace structure for nested files', async () => {
    ...
    assert.ok(existsSync(join(paths.workspace, 'guides', 'intro.md')), 'Should preserve nested structure')
    assert.ok(existsSync(join(paths.workspace, 'guides', 'advanced', 'tips.md')), 'Should preserve deep nesting')
  })
})
